...
public class day01_p2 {
	...
	public static int calculateWithExtraFull(final int mass) {
		int sum = 0;
		int added = calculate(mass);
		while (added > 0) {
			sum += added;
			added = calculate(added);
		}
		return sum;
	}

	public static int fuelSumFromMass(IntStream stream) {
		return stream.map(day01_p2::calculateWithExtraFull).sum();
	}
	...
	public static void main2(String[] args) throws URISyntaxException {
		...
		try (Stream<String> stream = Files.lines(Paths.get(filename));
			...
			stream.mapToInt(Integer::parseInt).map(day01_p2::calculate).forEach(arg0 -> {
				try {
